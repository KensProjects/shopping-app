...
export default function NavbarToggle() {
  ...
  const linkStyles = "flex flex-col justify-center items-center w-full h-1/5 hover:text-red-800 text-2xl hover:text-3xl ease-in-out duration-100"

  if (!mobileToggleIsActive) return null

  return (
    <div className="w-screen h-screen flex flex-col justify-center items-center fixed top-0 right-0 bg-black/50 z-50 overflow-auto">
      <ul className="w-64 h-full bg-white flex flex-col fixed top-0 right-0 z-50 justify-start items-center border border-black">
        ...
        {!session && <SignInButton callback={() => setMobileToggleIsActive(false)} />}
