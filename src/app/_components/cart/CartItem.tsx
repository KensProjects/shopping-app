...
export default function CartItem({ name, price, imageSrc, quantity }: Catalog) {
    ...
    return (
        ...
        <div className="flex rounded-sm justify-between items-center h-60 w-full p-2 text-center">
            <div className="item-icon flex flex-col justify-center items-center w-full h-full text-center">
                <Link href={`/catalog/${cartItemName}`}>
                    <Image src={imageSrc} alt={name} width={65} height={65} />
                </Link>

                <span className="item-name text:md sm:text-xl text-center">{name}</span>
                <span className="item-price text-md sm:text-lg">{cartItemPrice}</span>
            ...
            <p className="flex justify-center items-center text-center p-4 text-sm sm:text-lg">Lorem, ipsum dolor sit amet consectetur adipisicing elit</p>
        </div>
    )
}
